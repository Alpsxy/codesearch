...
def parse_option():
    ...
    # model definition
    parser.add_argument("--model", type=str, default="mpnn", choices=["gat", "mpnn", "gin"])
    ...
    # specify folder
    parser.add_argument("--model-path", type=str, default='ast_pretrain/saved_models/GCC_CSN', help="path to save model")
    ...
    # GPU setting
    parser.add_argument("--gpu", default=[1, 2, 3, 0], type=int, nargs='+', help="GPU id to use.")
...
def train_moco(
    epoch, train_loader, model_query, model, model_ema, contrast, criterion, optimizer, sw, args
):
    ...
    for idx, batch in enumerate(train_loader):
        ...
        # query.to(torch.device(args.gpu))
...
def main(args):
    ...
    model_query = QueryEncoder(len(LoadBalanceGraphDataset.query_vocab), args.query_emb_size, args.query_lstm_size, args.query_hidden_size)
